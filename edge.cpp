...
//int EdgeParent::_idStatic = 1;

EdgeParent::EdgeParent(Node *sourceNode, Node *destNode, QString text, bool is_show_id, bool is_directed)
    : NodeEdgeParent(sourceNode->graph, 0, text, is_show_id),
      source(sourceNode), dest(destNode),
      arrowSize(15), is_directed(is_directed)
{
    setTextContent("1");
    if (!dest) {
        dest = source;
    ...
    int max_id = -1;
    foreach (auto item, sourceNode->edges()){
        if (((item->sourceNode() == sourceNode) && (item->destNode() == destNode)) || ((item->sourceNode() == destNode) && (item->destNode() == sourceNode))) {
            max_id++;
        }
    }
    SetId(max_id);
...
Edge::Edge(Node *sourceNode, Node *destNode, QString text, bool is_show_id, bool is_directed)
    : EdgeParent(sourceNode, destNode, text, is_show_id, is_directed)
{
    ...
    // При вервом соединении - прямая
    int countCopy = -1;
    foreach (auto e, dest->edges()) {
        if (e->destNode() == source) {
            countCopy = 0;
        }
    }
...
void Edge::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget)
{
    ...
    painter->setPen(QPen((isSelected() ? Qt::cyan: _color), 2, Qt::SolidLine, Qt::RoundCap, Qt::RoundJoin));
    ...
    if (source != dest) {
       ...
       // myPath.lineTo(mapFromScene(dest->pos()));
...
EdgeCircle::EdgeCircle(Node *sourceNode, QString text, bool is_show_id, bool is_directed)
    : EdgeParent (sourceNode, sourceNode, text, is_show_id, is_directed)
